...
const Sidebar = () => {
  ...
  // Scroll automat la cel mai recent prompt
  useEffect(() => {
    if (recentContainerRef.current) {
      recentContainerRef.current.scrollTop = recentContainerRef.current.scrollHeight
    }
  }, [prevPrompts])
  ...
  return (
    <div className='hidden sm:inline-flex flex-col justify-between bg-gray-100 min-h-screen p-6 w-auto'>
      <div className="top">
        ...
        {extended &&
        <div 
          className="flex flex-col animate-fadeIn mt-8 overflow-y-auto max-h-64" 
          ref={recentContainerRef}
        >
          <p className="mb-5 text-gray-700 font-medium">Recent</p>
          {prevPrompts.map((item, index)=>{
            return (
              <div 
                onClick={()=>loadPrompt(item)} 
                key={index}
                className="flex items-start gap-2 px-4 py-2 rounded-full text-gray-800 cursor-pointer hover:bg-gray-200"
              >
